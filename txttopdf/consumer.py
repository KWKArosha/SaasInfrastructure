...
INPUT_TOPIC = os.getenv("KAFKA_INPUT_TOPIC", "demo-messages")
OUTPUT_TOPIC = os.getenv("KAFKA_OUTPUT_TOPIC", "pdf-results")
GROUP_ID = os.getenv("KAFKA_GROUP_ID", "txttopdf-converter")
...
def create_pdf(text: str, destination: Path) -> None:
...
def upload_pdf_to_minio(minio_client: Minio, bucket_name: str, file_path: Path) -> str:
...
def main() -> None:
    ...
    for record in consumer:
        ...
        try:
            create_pdf(text, destination)
            object_url = upload_pdf_to_minio(minio_client, MINIO_BUCKET, destination)
            result = {
                "status": "completed",
                "filename": filename,
            ...
            logger.info("Converted Kafka record %s:%s to %s and uploaded to %s", record.partition, record.offset, destination, object_url)
        except Exception as error:
            logger.exception("Could not convert Kafka record %s:%s", record.partition, record.offset)
